...
for row in d['bethe']:
    assert abs(row['bethe_abs_logZ_error']-row['predicted_logZ_error']) < 1e-12
    assert row['precision_cutoff_logZ_change'] < 1e-12
    assert row['quadrature_logZ_error'] < 1e-12
for row in d['descendant_integral_checks']:
    assert abs(row['E0_over_Z']-row['predicted_E0_over_Z']) < 1e-12
    assert abs(row['E1_over_Z']-row['predicted_E1_over_Z']) < 1e-12
assert d['bias_gradient_check']['max_abs_bias_gradient_error'] < 1e-8

def num(v): return 'failed (nonpositive Z)' if v is None else ('below 1e-13' if v<1e-13 else f'{v:.3e}')
lines=[r'''# Bethe/descent comparison: measured results and scope
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|''']
for x in d['bethe']:
    lines.append(f"| {x['model']} | {x['N']} | {x['beta']:g} | {num(x['bethe_abs_logZ_error'])} | {num(x['float64_residue_abs_logZ_error'])} | {x['float64_residue_seconds']*1000:.3f} | {x['bethe_seconds']*1000:.3f} | {x['hermite64_seconds']*1000:.3f} | {num(x['hermite64_logZ_error'])} |")
lines.append(r'''
...
|---:|---|---:|---:|---:|''')
for x in d['baselines']:
    err=x.get('abs_logZ_error',x.get('logZ_RMSE'))
    grad=x.get('max_abs_gradient_error',x.get('mean_max_abs_gradient_error'))
    secs=x.get('seconds',x.get('median_seconds'))
    lines.append(f"| {x['r']} | {x['method']} | {num(err)} | {num(grad)} | {secs*1000:.3f} |")
lines.append(r'''
...
print('All comparison checks passed; report.md written.')
