...
for c in cases:
    ...
    assert abs(c['actual_abs_logZ_error']-c['analytical_logZ_truncation'])<1e-10
    if 'cutoff_refinement_change' in c: assert c['cutoff_refinement_change']<5e-12
    if c['r']==2:
        assert c['literal_vs_batched_logZ_difference']<5e-12
        S=sum(math.exp(-c['N']*math.pi**2*l*(l+1)/(2*c['beta'])) for l in range(1,100))
        assert max(abs(t['relative_error']-S) for t in c['first_integration_trace'])<5e-12

lines=[r'''# Sequential residue integration: a working implementation
...
|---:|---:|---:|---:|---:|---:|---:|---:|''']
for c in cases:
    error='<1e-13' if c['actual_abs_logZ_error']<1e-13 else f"{c['actual_abs_logZ_error']:.3e}"
    lines.append(f"| {c['N']} | {c['r']} | {c['beta']:g} | {c['logZ']:.12f} | {error} | {c['analytical_logZ_truncation']:.3e} | {c['pole_combinations']:,} | {c['seconds']:.4g} s |")
lines.append(r'''
...
|---:|---:|---:|''')
c=next(c for c in cases if c['r']==2 and c['beta']==1)
for t in c['first_integration_trace']:
    lines.append(f"| {t['m2']:g} | {t['F1_real']:.12f} | {t['exact_partial']:.12f} |")
lines.append(f"\nApplying the m2 residue operation to F1 gives logZ={c['literal_sequential_logZ']:.15f}; the independent reference is {c['reference_logZ']:.15f}. The literal nested callables agree with the batched evaluation.\n")
lines.append(r'''
## Reproduce
...
print('Sequential stages, truncation identities and references checked; report.md written.')
