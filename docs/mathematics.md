...
For $b_i(h)=b_i+h a_i$, let $M=N^{-1}\sum_i a_i\sigma_i$, $u_i=\beta(b_i+h a_i+W_i\cdot x)$, $T=\sum_i a_i\tanh u_i$, and $V=\sum_i a_i^2\operatorname{sech}^2u_i$. With positive normalized lattice weights,
